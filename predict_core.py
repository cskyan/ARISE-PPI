...
from .model import L13PDBGVPModel  # noqa: E402
from . import train_core as trainlib  # noqa: E402
from .config import Params, build_model_config as _fallback_build_model_config  # noqa: E402
...
_SITE_HEAD_FOR_PREDICT = str(
    os.environ.get("site_head_type", os.environ.get("SITE_HEAD_TYPE", getattr(P, "site_head_type", "amleh")))
).strip().lower()
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
USE_AMP = (os.environ.get("AMP", "1").strip().lower() not in ("0", "false", "no", "off", "")) and torch.cuda.is_available() and autocast is not None
AMP_DTYPE = torch.bfloat16 if (torch.cuda.is_available() and torch.cuda.is_bf16_supported()) else torch.float16
METRIC_FIELDS = ["acc", "recall", "precision", "f1", "mcc", "auroc", "auprc"]
TOPK_FIELDS = [
...
def make_embedder():
    if str(getattr(P, "sequence_mode", "esm")).lower() not in ("esm", "hybrid"):
        return None
    try:
        return trainlib.SiteEmbedder(
            device=DEVICE,
            esm_local_dir=getattr(P, "esm_local_dir", getattr(trainlib, "ESM_LOCAL_DIR", "")),
        )
...
def load_model(checkpoint: str, d_res: int, d_chain: int):
    ...
    if cfg is None:
        build_cfg = getattr(trainlib, "build_model_config", _fallback_build_model_config)
        cfg = build_cfg(P, d_res, d_chain)
    if hasattr(trainlib, "_ensure_model_config_compat"):
        cfg = trainlib._ensure_model_config_compat(cfg, P)
    ckpt_d = int(getattr(cfg, "d_seq_in", d_res))
    if ckpt_d != int(d_res):
        raise RuntimeError(
            "Checkpoint/input feature dimension mismatch: "
            f"checkpoint d_seq_in={ckpt_d}, current data d_res={int(d_res)}. "
            "This usually means you used use_hmm=1 with a non-HMM checkpoint, "
            "or loaded an HMM checkpoint without use_hmm=1. "
...
def predict_forward(model, resA, maskA, cA, coordsA, resB, maskB, cB, coordsB):
    try:
        with (autocast(dtype=AMP_DTYPE) if USE_AMP else nullcontext()):
            return model(resA, maskA, cA, coordsA, resB, maskB, cB, coordsB, site_mode=True)
    except RuntimeError as e:
        msg = str(e).lower()
        if "cudnn" not in msg or "algorithm" not in msg:
            raise
        print("[predict][warn] cuDNN convolution failed; retrying this batch with cuDNN disabled.", flush=True)
...
def collect_labeled_scores(model, dl) -> Tuple[np.ndarray, np.ndarray]:
    ...
    with torch.no_grad():
        for batch in dl:
            ...
            out = predict_forward(model, resA, maskA, cA, coordsA, resB, maskB, cB, coordsB)
            probs = torch.sigmoid(out["logit_resA"]).detach().float().cpu()
...
def write_metrics_tsv(path: str, metrics: Dict):
    header = ["ACC", "Recall", "Precision", "F1", "MCC", "AUC", "PRC", "Threshold", "N_eval", "Pos_rate"]
    keys = ["acc", "recall", "precision", "f1", "mcc", "auroc", "auprc"]
    vals = []
    for k in keys:
        try:
...
def write_metrics_comparison_tsv(path: str, checkpoint_metrics: Dict, auto_current: Dict):
    rows = []
    for name, metrics in [("checkpoint_val_threshold", checkpoint_metrics), ("auto_current_label_threshold", auto_current)]:
        row = {"setting": name}
...
def resolve_checkpoint(args) -> str:
    ...
    if tag == "auto":
        ...
        tag = "TOPK"
    elif tag in ("pair", "ppi", "pair_auprc", "best_pair"):
        tag = "PAIR_AUPRC"
    elif tag in ("auc", "auroc", "binary", "best_auc"):
        tag = "AUPRC"
    elif tag in ("auprc", "best_auprc"):
        tag = "AUPRC"
    ckpt = os.path.join(P.save_dir, "checkpoints", "best_%s.pt" % tag)
    if not os.path.exists(ckpt) and tag == "AUPRC":
        ckpt_dir = os.path.join(P.save_dir, "checkpoints")
        try:
            stamped = [
                os.path.join(ckpt_dir, fn)
                for fn in os.listdir(ckpt_dir)
                if fn.startswith("best_AUPRC_") and fn.endswith(".pt")
            ]
        except Exception:
            stamped = []
        if stamped:
            stamped.sort(key=lambda x: os.path.getmtime(x), reverse=True)
            print(
                "[predict][warn] best_AUPRC.pt not found; using latest timestamped checkpoint: %s"
                % stamped[0],
                flush=True,
            )
            return stamped[0]
...
def predict(args):
    ...
    if args.threshold is not None and float(args.threshold) >= 0:
        ...
        try:
            ...
            calibration_metrics["calibration_split"] = val_path
    ...
    write_protocol_metrics(out_dir, metrics, topk_metrics)
    ...
    if str(getattr(P, "primary_objective", "binary")).lower() == "topk":
        print("[predict][topk] checkpoint=%s metrics=%s" % (os.path.basename(ckpt), compact_topk_metrics(topk_metrics)), flush=True)
        print("[predict][binary-diagnostic] threshold=%.4f source=%s metrics=%s" % (thr, threshold_source, compact_metrics(metrics)), flush=True)
    else:
        print("[predict] threshold=%.4f source=%s metrics=%s" % (thr, threshold_source, compact_metrics(metrics)), flush=True)
...
def write_protocol_metrics(out_dir: str, metrics: Dict, topk_metrics: Dict):
    row = {
        "split": "current",
        "checkpoint": os.path.basename(str(metrics.get("checkpoint", ""))),
        "recall_L5": float(topk_metrics.get("recall_L5", 0.0)),
        "recall_L10": float(topk_metrics.get("recall_L10", 0.0)),
        "precision_K10": float(topk_metrics.get("precision_K10", 0.0)),
        "hit_K20": float(topk_metrics.get("hit_K20", 0.0)),
        "enrichment_K10": float(topk_metrics.get("enrichment_K10", 0.0)),
        "topk_score": float(topk_metrics.get("topk_score", 0.0)),
        "ager": int(bool(topk_metrics.get("ager", False))),
        "binary_auroc_diagnostic": float(metrics.get("auroc", 0.0)),
        "binary_auprc_diagnostic": float(metrics.get("auprc", 0.0)),
        "n_eval_residues": int(metrics.get("n_eval", 0)),
        "n_proteins": int(metrics.get("n_proteins", 0)),
        "n_labeled_proteins": int(metrics.get("n_labeled_proteins", 0)),
    }
...
def compact_metrics(metrics: Dict) -> str:
    keys = ["acc", "recall", "precision", "f1", "mcc", "auroc", "auprc", "n_eval"]
    parts = []
    for k in keys:
        if k in metrics:
            v = metrics[k]
...
def write_case_study(out_dir: str, protein_rows: List[Dict], ranking_rows: List[Dict], metrics: Dict, args):
    ...
    if "auroc" in metrics:
        lines.append(
            "- Labeled-residue validation: ACC %.4f, Recall %.4f, Precision %.4f, F1 %.4f, MCC %.4f, AUC %.4f at threshold %.3f\n"
            % (
                metrics.get("acc", 0.0),
...
def main():
    ...
    parser.add_argument("--checkpoint-tag", default="auto",
                        choices=["auto", "topk", "pair", "ppi", "pair_auprc", "auprc", "auc", "auroc", "binary"],
    ...
    parser.add_argument("--threshold-mode", default="val_recall_floor",
    ...
    parser.add_argument("--ager-scan-apply-best", action="store_true",
                        help="After --ager-scan, apply the best scanned AGER parameters to exported rankings and top-k metrics.")
    ...
    cli_args = sys.argv[1:]
    has_opt = lambda name: any(x == name or x.startswith(name + "=") for x in cli_args)
    if args.root:
        P.rbp_root = str(args.root)
        if not has_opt("--id-list"):
            P.rbp_id_list = os.path.join(P.rbp_root, "all_ids.txt")
        if not has_opt("--train-list"):
            P.rbp_train_list = os.path.join(P.rbp_root, "train.txt")
        if not has_opt("--val-list"):
            P.rbp_val_list = os.path.join(P.rbp_root, "val.txt")
        if not has_opt("--test-list"):
            P.rbp_test_list = os.path.join(P.rbp_root, "test.txt")
    predict(args)
