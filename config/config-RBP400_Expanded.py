...
SERVER_RBP_ROOT = "/srv/storage1/ssd/ysk/jiangbo/PhD/new4/RBP400"
SIBLING_RBP_ROOT = os.path.join(
    os.path.dirname(PROJECT_ROOT), "new4", "RBP400"
)
DEFAULT_RBP_ROOT = os.path.abspath(
    os.environ.get(
        "RBP400_ROOT",
        next(
            (
                ...
                for path in (
                    SERVER_RBP_ROOT,
                    SIBLING_RBP_ROOT,
                    os.path.join(PROJECT_ROOT, "RBP400"),
                )
...
class Params:
    ...
    rbp_root: str = DEFAULT_RBP_ROOT
    rbp_id_list: str = os.path.join(DEFAULT_RBP_ROOT, "all_ids.txt")
    rbp_train_list: str = os.path.join(DEFAULT_RBP_ROOT, "train.txt")
    rbp_val_list: str = os.path.join(DEFAULT_RBP_ROOT, "val.txt")
    rbp_test_list: str = os.path.join(DEFAULT_RBP_ROOT, "test.txt")
    ...
    rbp_label_tier: str = "auto"  # auto | gold | silver | legacy
    rbp_require_label_masks: bool = True
    ...
    rbp_dssp_dir: str = "dssp_rsa_asa"
    dssp_dim: int = 2              # use 2 for [ASA, RSA]; legacy one-hot uses 9
    surface_mask_dir: str = "surface_mask_rsa05"
    binary_primary_metric: str = "auprc"  # auprc | auprc_macro | ca12_auprc | surface_auprc
    ...
    use_pssm: bool = True
    use_dssp: bool = True
    sequence_mode: str = "hybrid"       # esm | light | hybrid
    ...
    # auto: L3/pair task uses pair-decision-first loss; DEST/binary uses L1-first loss.
    ...
    # In L1-main/DEST mode, an optional bag-level L3 auxiliary can be used.
    ...
    # l2_map_w_start is a multiplier in [0, 1], not an absolute weight.
    # l2_w_eff = l2_map_w * (l2_start + (1 - l2_start) * ramp_factor)
    # ep00 (factor=1/6): 3.0*(0.15+0.85*0.167)=0.88.
    # ep06+ (factor=1): 3.0*(0.15+0.85*1.0)=3.0.
    ...
    pair_head_type: str = "eb"  # eb | no_eb | attention | mean | eb_l2 | gpeh
    eb_proposal_mode: str = "top"  # top | random | low | shuffled
    eb_support_mode: str = "top"  # top | all_candidates
    eb_support_temperature: float = 1.0
    eb_random_seed: int = 1337
    ...
    def from_env(cls, p=None):
        ...
        spec = [
            ...
            ("rbp_label_tier", _env_str), ("rbp_require_label_masks", _env_bool),
            ...
            ("eb_proposal_mode", _env_str), ("eb_support_mode", _env_str),
            ("eb_support_temperature", _env_float), ("eb_random_seed", _env_int),
        ...
        p.pair_head_type = str(getattr(p, "pair_head_type", "eb")).lower()
        p.eb_proposal_mode = str(getattr(p, "eb_proposal_mode", "top")).lower()
        p.eb_support_mode = str(getattr(p, "eb_support_mode", "top")).lower()
...
class ModelConfig:
    ...
    eb_dropout: float = 0.10
    eb_use_pair_context: bool = True
    pair_head_type: str = "eb"
    eb_proposal_mode: str = "top"
    eb_support_mode: str = "top"
    eb_support_temperature: float = 1.0
    eb_random_seed: int = 1337
    pair_transformer_layers: int = 2
    pair_transformer_heads: int = 4
...
def build_model_config(p: Params, d_seq_in: int, d_chain_in: int) -> ModelConfig:
    return ModelConfig(
        ...
        pair_head_type=str(getattr(p, "pair_head_type", "eb")).lower(),
        eb_proposal_mode=str(getattr(p, "eb_proposal_mode", "top")).lower(),
        eb_support_mode=str(getattr(p, "eb_support_mode", "top")).lower(),
        eb_support_temperature=float(getattr(p, "eb_support_temperature", 1.0)),
        eb_random_seed=int(getattr(p, "eb_random_seed", 1337)),
