...
from config_L131 import ModelConfig
...
class FiLM(nn.Module):
    """Chain-level feature-wise linear modulation."""
...
class SelfEnc(nn.Module):
    """Stacked pre-layer-normalized Transformer encoder."""
...
class CrossBlock(nn.Module):
    """Dual-chain cross-attention, self-attention, and FFN."""
...
class FragmentHead(nn.Module):
    ...
    def __init__(self, d_model: int, kernel_size: int = 9, dropout: float = 0.1):
        ...
        # BatchNorm1d depends on batch statistics, which is unstable with B=1,
...
# ============================================================
# EvidenceBridge: L1 residue evidence to L3 pair evidence
# ============================================================


class EvidenceBridge(nn.Module):
    """
    Aggregate L1 residue evidence into pair-aware residue-pair evidence and
    compress it into the evidence vector consumed by L3.

    The bridge first selects residues on each chain, builds the Cartesian
    candidate score matrix, and returns an explicit retained-support ranking.
    """
    ...
    def _topk_select(
        ...
        if fixed_idx is not None:
            idx = fixed_idx.to(device=logits.device, dtype=torch.long)
            idx = idx[:, :K].clamp(min=0, max=max(0, L - 1))
        else:
            mode = str(proposal_mode or "top").lower()
            valid = torch.ones_like(logits, dtype=torch.bool) if mask is None else _bmask(mask)
            if mode == "top":
                selection_score = logits.clone()
            elif mode == "low":
                selection_score = -logits
            elif mode == "random":
                selection_score = torch.rand(
                    logits.shape, dtype=logits.dtype, device=logits.device
                )
            elif mode == "shuffled":
                selection_score = torch.empty_like(logits)
                for b in range(B):
                    selection_score[b] = logits[b, torch.randperm(L, device=logits.device)]
            else:
                raise ValueError(f"Unknown EB proposal_mode={proposal_mode!r}")
            selection_score = selection_score.masked_fill(~valid, -1e9)
            idx = torch.topk(selection_score, k=K, dim=1, largest=True, sorted=True).indices

        val = logits.gather(1, idx)
    ...
    def forward(
            ...
            proposal_mode: str = "top",
            support_mode: str = "top",
            support_temperature: float = 1.0,
            fixed_selection: Optional[Dict] = None,
            support_keep_mask: Optional[torch.Tensor] = None,
        ...
        mode = str(support_mode or "top").lower()
        fixed_flat = fixed_selection.get("retained_pair_flat")
        if fixed_flat is None and (
            fixed_selection.get("pair_idxA_local") is not None
            and fixed_selection.get("pair_idxB_local") is not None
        ):
            fixed_flat = (
                fixed_selection["pair_idxA_local"].to(flat.device) * KB
                + fixed_selection["pair_idxB_local"].to(flat.device)
            )

        if fixed_flat is not None:
            top_pair_flat = fixed_flat.to(device=flat.device, dtype=torch.long)
            top_pair_flat = top_pair_flat.clamp(min=0, max=max(0, flat.size(1) - 1))
            top_pair_val = flat.gather(1, top_pair_flat)
            mode = "top"
        elif mode == "top":
            pair_k = max(1, min(self.pair_k, flat.size(1)))
            top_pair_val, top_pair_flat = torch.topk(
                flat, k=pair_k, dim=1, largest=True, sorted=True
            )
        elif mode == "all_candidates":
            top_pair_val = flat
            top_pair_flat = torch.arange(
                flat.size(1), device=flat.device, dtype=torch.long
            ).unsqueeze(0).expand(B, -1)
        else:
            raise ValueError(f"Unknown EB support_mode={support_mode!r}")

        pair_idxA_local = torch.div(top_pair_flat, KB, rounding_mode="floor")
        pair_idxB_local = top_pair_flat % KB

        temperature = max(1e-6, float(support_temperature))
        support_logits = top_pair_val / temperature
        empty_support = torch.zeros(B, dtype=torch.bool, device=flat.device)
        if support_keep_mask is not None:
            keep = support_keep_mask.to(device=flat.device, dtype=torch.bool)
            keep = keep[:, :support_logits.size(1)]
            if keep.size(1) < support_logits.size(1):
                keep = F.pad(keep, (0, support_logits.size(1) - keep.size(1)), value=True)
            empty_support = ~keep.any(dim=1)
            support_logits = support_logits.masked_fill(~keep, -torch.inf)
        pair_weight = torch.nan_to_num(
            torch.softmax(support_logits, dim=1),
            nan=0.0,
            posinf=0.0,
            neginf=0.0,
        )

        if mode == "all_candidates" and fixed_flat is None:
            weight_matrix = pair_weight.reshape(B, KA, KB)
            weighted_A = torch.einsum("bij,bid->bd", weight_matrix, topk_fA)
            weighted_B = torch.einsum("bij,bjd->bd", weight_matrix, topk_fB)
            evi_vec = torch.cat([weighted_A, weighted_B], dim=-1)
        else:
        ...
        topk_info = {
            ...
            "retained_pair_flat": top_pair_flat,
        ...
        if export_candidates:
            topk_info.update({
                "candidate_idxA": topk_idxA.unsqueeze(2).expand(-1, -1, KB).reshape(B, -1),
                "candidate_idxB": topk_idxB.unsqueeze(1).expand(-1, KA, -1).reshape(B, -1),
                "candidate_score": flat,
            })
        return evi_vec, evi_score, topk_info


class AttentionEvidencePool(nn.Module):
    """Parameterized residue attention control with the same EB output width."""

    def __init__(self, d_model: int, d_proj: int = 64, dropout: float = 0.1):
        super().__init__()
        self.projA = nn.Sequential(
            nn.LayerNorm(d_model), nn.Linear(d_model, d_proj), nn.GELU()
        )
        self.projB = nn.Sequential(
            nn.LayerNorm(d_model), nn.Linear(d_model, d_proj), nn.GELU()
        )
        self.scoreA = nn.Sequential(
            nn.LayerNorm(d_model), nn.Linear(d_model, d_proj), nn.Tanh(),
            nn.Linear(d_proj, 1),
        )
        self.scoreB = nn.Sequential(
            nn.LayerNorm(d_model), nn.Linear(d_model, d_proj), nn.Tanh(),
            nn.Linear(d_proj, 1),
        )
        ...
        self.evi_scalar = nn.Linear(d_proj * 2, 1)

    @staticmethod
    def _pool(x, proj, score, mask):
        logits = score(x).squeeze(-1)
        if mask is not None:
            logits = logits.masked_fill(~_bmask(mask), -1e9)
        weight = torch.softmax(logits, dim=1)
        pooled = (proj(x) * weight.unsqueeze(-1)).sum(dim=1)
        return pooled, weight

    def forward(self, xA, xB, maskA=None, maskB=None):
        pooled_A, weight_A = self._pool(xA, self.projA, self.scoreA, maskA)
        pooled_B, weight_B = self._pool(xB, self.projB, self.scoreB, maskB)
        evi_vec = self.out(torch.cat([pooled_A, pooled_B], dim=-1))
        evi_score = self.evi_scalar(evi_vec).squeeze(-1)
        return evi_vec, evi_score, {
            "attention_weightA": weight_A,
            "attention_weightB": weight_B,
        }


class MeanEvidencePool(nn.Module):
    """Mean/concatenation pooling control with the same EB output width."""
    ...
    def _mean(x, mask):
        if mask is None:
            return x.mean(dim=1)
        weight = _bmask(mask).float().unsqueeze(-1)
        return (x * weight).sum(dim=1) / weight.sum(dim=1).clamp_min(1.0)

    def forward(self, xA, xB, maskA=None, maskB=None):
        pooled_A = self.projA(self._mean(xA, maskA))
        pooled_B = self.projB(self._mean(xB, maskB))
        evi_vec = self.out(torch.cat([pooled_A, pooled_B], dim=-1))
        evi_score = self.evi_scalar(evi_vec).squeeze(-1)
        return evi_vec, evi_score, {}
...
# ============================================================
# L3 head: pair decision from global and bridge evidence
# ============================================================
...
class L3Head(nn.Module):
    ...
    Output:
      ...
      EvidenceBridge separately provides the explanatory evidence score.
    """

    def __init__(self, d_model: int, d_proj: int = 64, d_hidden: int = 256,
                 n_layers: int = 2, dropout: float = 0.2):
        super().__init__()
        # Concatenate [gA, gB, evi_vec] and classify with an MLP.
...
class L13PDBGVPModel(nn.Module):
    def __init__(self, cfg: ModelConfig):
        ...
        self.attention_bridge = AttentionEvidencePool(
            d_model=D, d_proj=cfg.eb_d_proj, dropout=DR
        )
        self.mean_bridge = MeanEvidencePool(
            d_model=D, d_proj=cfg.eb_d_proj, dropout=DR
        )
    ...
    def _global_pool(x, mask):
        ...
        return (x*m).sum(dim=1) / m.sum(dim=1).clamp_min(1.0)
    def forward(
            self, resA, maskA, chainA, coordsA,
            resB, maskB, chainB, coordsB,
        ...
        pair_head_type = str(
            intervention.get(
                "pair_head_type",
                getattr(self.cfg, "pair_head_type", "eb"),
            )
        ).lower()
        global_A = self._global_pool(xA, maskA)
        global_B = self._global_pool(xB, maskB)

        if bool(getattr(self.cfg, "eb_enable", True)) and pair_head_type == "gpeh":
            ...
            return {
                ...
                'pair_weight': xA.new_empty((xA.size(0), 0)),
                'model_metadata': {
                    'site_mode': False, 'pair_head_type': 'gpeh',
                    'l2_active': False, 'gc_eb_active': True,
                    'native_eb_export': False,
                },
            }

        if pair_head_type == "no_eb":
            evi_vec = xA.new_zeros((xA.size(0), int(self.cfg.eb_d_proj) * 2))
            evi_score = xA.new_zeros((xA.size(0),))
            topk = {}
        elif pair_head_type == "attention":
            evi_vec, evi_score, topk = self.attention_bridge(
                xA, xB, maskA, maskB
            )
        elif pair_head_type == "mean":
            evi_vec, evi_score, topk = self.mean_bridge(
                xA, xB, maskA, maskB
            )
        elif pair_head_type not in ("eb", "eb_l2"):
            raise ValueError(
                f"Unknown pair_head_type={pair_head_type!r}; expected "
                "eb, no_eb, attention, mean, eb_l2, or gpeh"
            )
        else:
            ep_now = int(getattr(self, 'training_epoch', 0))
            eb_start = int(getattr(self.cfg, 'eb_start_epoch', 0))
            eb_ramp = int(getattr(self.cfg, 'eb_ramp_epochs', 0))
            if not self.training:
                eb_ramp_factor = xA.new_tensor(1.0)
            elif ep_now < eb_start:
                eb_ramp_factor = xA.new_tensor(0.0)
            elif eb_ramp <= 0:
                eb_ramp_factor = xA.new_tensor(1.0)
            else:
                eb_ramp_factor = xA.new_tensor(
                    min(1.0, float(ep_now - eb_start + 1) / float(max(1, eb_ramp)))
                )
            eb_gate = torch.sigmoid(self.eb_gate_scale) * eb_ramp_factor
            evi_vec, evi_score, topk = self.evidence_bridge(
                logit_resA, logit_resB, xA, xB, maskA, maskB, eb_gate,
                proposal_mode=str(
                    intervention.get(
                        "proposal_mode",
                        getattr(self.cfg, "eb_proposal_mode", "top"),
                    )
                ).lower(),
                support_mode=str(
                    intervention.get(
                        "support_mode",
                        getattr(self.cfg, "eb_support_mode", "top"),
                    )
                ).lower(),
                support_temperature=float(
                    intervention.get(
                        "support_temperature",
                        getattr(self.cfg, "eb_support_temperature", 1.0),
                    )
                ),
                fixed_selection=intervention.get("fixed_selection"),
                support_keep_mask=intervention.get("support_keep_mask"),
                export_candidates=bool(intervention.get("export_candidates", False)),
            )

        empty_long = torch.empty(
            (xA.size(0), 0), dtype=torch.long, device=xA.device
        )
        empty_float = xA.new_empty((xA.size(0), 0))
        l2_active = pair_head_type == "eb_l2"
        l2_score = None
        S = None
        l2_info = None
        pair_evidence = evi_vec
        if l2_active:
            S, l2_vec, l2_score, l2_info = self.l2_bridge(
                logit_resA, logit_resB, xA, xB, maskA, maskB, coordsA, coordsB
            )
            pair_evidence = self.l2_to_l3(torch.cat([evi_vec, l2_vec], dim=-1))

        pair_logit = self.l3_head(global_A, global_B, pair_evidence)
        if l2_active:
            pair_logit = pair_logit + torch.tanh(self.l2_pair_scale) * l2_score

        topk_idxA = topk.get("topk_idxA", empty_long)
        topk_idxB = topk.get("topk_idxB", empty_long)
        topk_valA = topk.get("topk_valA", empty_float)
        topk_valB = topk.get("topk_valB", empty_float)
        pair_idxA = topk.get("pair_residue_idxA", empty_long)
        pair_idxB = topk.get("pair_residue_idxB", empty_long)
        pair_score = topk.get("pair_score", empty_float)
        pair_weight = topk.get("pair_weight", empty_float)
        pair_compactness = topk.get(
            "pair_compactness", xA.new_zeros((xA.size(0),))
        )
        metadata = {
            'site_mode': False,
            'pair_head_type': pair_head_type,
            'l2_active': bool(l2_active),
            'gc_eb_active': False,
        ...
        self._cache = {
            'logit_resA': logit_resA, 'logit_resB': logit_resB,
            'logit_fragA': logit_fragA, 'logit_fragB': logit_fragB,
            'encoded_A': xA, 'encoded_B': xB,
            'global_A': global_A, 'global_B': global_B,
            'evi_vec': pair_evidence, 'evi_score': evi_score,
            'l2_score': l2_score, 'S': S,
            'pair_logit': pair_logit, 'topk_info': topk,
            'l2_info': l2_info, 'model_metadata': metadata,
        }
        out = {
            'pair_logit': pair_logit,
            'pair_prob': torch.sigmoid(pair_logit),
            'evi_score': evi_score,
            'evi_prob': torch.sigmoid(evi_score),
            'logit_resA': logit_resA, 'logit_resB': logit_resB,
            'p_res_A': torch.sigmoid(logit_resA),
            'p_res_B': torch.sigmoid(logit_resB),
            'logit_fragA': logit_fragA, 'logit_fragB': logit_fragB,
            'p_frag_A': torch.sigmoid(logit_fragA),
            'p_frag_B': torch.sigmoid(logit_fragB),
            'topk_idxA': topk_idxA, 'topk_idxB': topk_idxB,
            'topk_valA': topk_valA, 'topk_valB': topk_valB,
            'pair_idxA': pair_idxA, 'pair_idxB': pair_idxB,
            'pair_score': pair_score, 'pair_weight': pair_weight,
            'pair_compactness': pair_compactness,
            'model_metadata': metadata,
        }
        for key in (
            "pair_idxA_local", "pair_idxB_local", "retained_pair_flat",
            "candidate_idxA", "candidate_idxB", "candidate_score",
        ):
            if key in topk:
                out[key] = topk[key]
        if l2_active:
            out.update({
                'l2_score': l2_score,
                'l2_prob': torch.sigmoid(l2_score),
                'S': S,
            })
        return out
