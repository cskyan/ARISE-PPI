...
from .config import Params, build_model_config
...
def _force_rbp400_binary_params(p):
    ...
    p.report_topk_metrics = str(os.environ.get("REPORT_TOPK_METRICS", "1")).strip().lower() not in ("0", "false", "no", "off", "")
    p.eval_tta_swap = False
    p.train_neg_ratio = float(os.environ.get("TRAIN_NEG_RATIO", getattr(p, "train_neg_ratio", 1.0)))
    ...
    p.site_self_cross = str(os.environ.get("SITE_SELF_CROSS", str(int(bool(getattr(p, "site_self_cross", False)))))).strip().lower() not in ("0", "false", "no", "off", "")
    p.site_graph_layers = int(os.environ.get("SITE_GRAPH_LAYERS", getattr(p, "site_graph_layers", 0)))
...
METRIC_FIELDS = [
    "acc", "recall", "precision", "f1", "mcc", "auroc",
]
TOPK_FIELDS = [
...
PAIR_FIELDS = [
    "pair_acc", "pair_precision", "pair_recall", "pair_f1", "pair_auprc", "pair_mcc",
...
def _load_hmm(path, length):
    arr = _load_npz_tensor(path, ("hmm", "HHM", "data"))
    if arr is None:
        return None
    if arr.ndim == 1:
        arr = arr.unsqueeze(-1)
    if arr.ndim != 2 or arr.shape[-1] < 30:
        return None
    arr = arr[:length, :30].float()
    # PP prepared HMM stores HHblits-style costs scaled to [0, 1]; lower is
    # stronger evidence, and '*' / missing values are near 1. Convert it into
    # an evidence-like feature where missing is near 0 before feeding the model.
    finite = torch.isfinite(arr)
    if bool(finite.any()):
        vmax = float(arr[finite].max().detach().cpu())
        vmin = float(arr[finite].min().detach().cpu())
        if vmin >= -1e-6 and vmax <= 1.0 + 1e-6:
            arr = 1.0 - arr.clamp(0.0, 1.0)
        else:
            arr = torch.exp(-arr.clamp(min=0.0) / 10000.0)
...
def _feature_coverage(root: str, ids: List[str]) -> Dict[str, int]:
    ...
    hmm_dir = _first_existing_dir(str(root), ['hmm', 'HMM', 'hhm', 'HHM'])
    ss_dir = _first_existing_dir(str(root), ['ss', 'SS'])
    out = {'n': len(ids), 'pssm': 0, 'pssm_nonzero': 0, 'dssp': 0, 'dssp_noncoil': 0, 'hmm': 0, 'ss': 0}
    for pid in ids:
        pssm_path = _first_existing_file([
        ...
        if pssm_path is not None:
            out['pssm'] += 1
            try:
                pssm_arr = _load_npz_tensor(pssm_path, ("pssm", "PSSM", "data"))
                if pssm_arr is not None and float(torch.as_tensor(pssm_arr).abs().max().item()) > 1e-8:
                    out['pssm_nonzero'] += 1
            except Exception:
                pass
        dssp_path = _first_existing_file([
        ...
        if dssp_path is not None:
            out['dssp'] += 1
            try:
                dssp_arr = _load_npz_tensor(dssp_path, ("dssp", "DSSP", "ss", "rsa_asa", "asa_rsa", "data"))
                if dssp_arr is not None:
                    dssp_arr = torch.as_tensor(dssp_arr)
                    # Legacy 9-dim DSSP uses H,E,B,G,I,T,S,- one-hot + exposure.
                    # All-coil placeholder DSSP has zero signal in columns 0:7.
                    if dssp_arr.ndim == 2 and dssp_arr.shape[-1] >= 8 and float(dssp_arr[:, :7].abs().sum().item()) > 1e-8:
                        out['dssp_noncoil'] += 1
            except Exception:
                pass
        if _first_existing_file([
            os.path.join(hmm_dir, f'{pid}.npy'),
            os.path.join(hmm_dir, f'{pid}.npz'),
            os.path.join(hmm_dir, f'{pid}.npz.gz'),
        ]) is not None:
            out['hmm'] += 1
        if _first_existing_file([
            os.path.join(ss_dir, f'{pid}.npy'),
            os.path.join(ss_dir, f'{pid}.npz'),
            os.path.join(ss_dir, f'{pid}.npz.gz'),
        ]) is not None:
            out['ss'] += 1
    return out
...
class RBP296Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, rbp_root, pid_list, embedder=None, use_pssm=True, use_dssp=True,
                 use_hmm=False, use_ss=False, esm_cache_dir=None, verbose=False):
        ...
        self.dir_pssm = _first_existing_dir(self.root, ['pssm', 'PSSM'])
        self.dir_dssp = _resolve_dssp_dir(self.root)
        self.dir_hmm = _first_existing_dir(self.root, ['hmm', 'HMM', 'hhm', 'HHM'])
        self.dir_ss = _first_existing_dir(self.root, ['ss', 'SS'])
    ...
    def __getitem__(self, idx: int):
        ...
        if self.use_dssp:
            ...
            dssp_dim = int(d.shape[-1]) if d is not None else _configured_dssp_dim(9)
            feats.append(_fit_len_feat(d, L, dssp_dim))
        if self.use_hmm:
            h = _load_hmm(_first_existing_file([
                os.path.join(self.dir_hmm, f'{pid}.npy'),
                os.path.join(self.dir_hmm, f'{pid}.npz'),
                os.path.join(self.dir_hmm, f'{pid}.npz.gz'),
            ]), L)
            feats.append(_fit_len_feat(h, L, 30))
        if self.use_ss:
            ss = _load_dssp(_first_existing_file([
                os.path.join(self.dir_ss, f'{pid}.npy'),
                os.path.join(self.dir_ss, f'{pid}.npz'),
                os.path.join(self.dir_ss, f'{pid}.npz.gz'),
            ]), L)
            ss_dim = int(ss.shape[-1]) if ss is not None else 9
            feats.append(_fit_len_feat(ss, L, ss_dim))
...
def _task_profile(site_mode: bool):
    ...
    L1/site task: residue-site decision dominates; L3 bag evidence is auxiliary.
    """
...
def forward_one(model: L13PDBGVPModel, batch: Dict, ep: int
    ...
    # L1/site task keeps residue-site prediction primary.
...
def eval_binary(model, dl, device=DEVICE, fixed_thr=None, threshold_mode=None):
    ...
    try:
        ...
        if site_detected:
            ...
            if lab.size and len(np.unique(lab)) > 1:
                ...
                res.update({'acc':0.0,'precision':0.0,'recall':0.0,'f1':0.0,'mcc':0.0,'auroc':0.0,'auprc':0.0,'thr':0.5,'primary':0.0})
            if bool(getattr(P, "report_topk_metrics", True)):
        ...
        if pdl.size and len(np.unique((pdl > 0.5).astype(np.int32))) > 1:
            ...
            thr_pair, thr_mode_pair = _select_binary_threshold(pdp, ypair, mode=getattr(P, 'val_thr_mode', 'auto_mcc'))
            ...
            res.update(dict(pair_auroc=0.0, pair_auprc=0.0, pair_acc=0.0, pair_precision=0.0, pair_recall=0.0, pair_f1=0.0, pair_mcc=0.0, pair_thr=0.5))
    ...
    with torch.no_grad():
        for batch in dl:
            ...
            try:
                ...
                msg = str(e).lower()
                if "cudnn" not in msg or "algorithm" not in msg:
                    raise
                print("[eval-topk][warn] cuDNN convolution failed; retrying this batch with cuDNN disabled.", flush=True)
...
def build_loaders():
    ...
    if use_rbp_dataset and os.path.isdir(os.path.join(DIPS_ROOT, 'labels')) and (
        os.path.exists(P.rbp_id_list) or
        (getattr(P, 'rbp_train_list', '') and os.path.exists(str(P.rbp_train_list)))
    ):
        has_prepared_splits = (
            os.path.exists(str(getattr(P, 'rbp_train_list', ''))) and
            os.path.exists(str(getattr(P, 'rbp_val_list', ''))) and
            os.path.exists(str(getattr(P, 'rbp_test_list', '')))
        )
        if not has_prepared_splits:
        ...
        pro_train_path = os.path.join(str(DIPS_ROOT), "pro_train.txt")
        if (not tr_ids or not va_ids) and os.path.exists(pro_train_path) and os.path.exists(str(getattr(P, 'rbp_test_list', ''))):
            pro_ids = _read_id_list_optional(pro_train_path)
            rng = random.Random(getattr(P, 'split_seed', 42))
            rng.shuffle(pro_ids)
            n_va = max(1, int(round(len(pro_ids) * float(getattr(P, 'split_val', 0.10)))))
            n_va = min(n_va, max(1, len(pro_ids) - 1))
            va_ids = pro_ids[:n_va]
            tr_ids = pro_ids[n_va:]
            print(
        ...
        ds_tr = RBP296Dataset(DIPS_ROOT, tr_ids, embedder=emb, use_pssm=P.use_pssm, use_dssp=P.use_dssp,
                              use_hmm=getattr(P, 'use_hmm', False), use_ss=getattr(P, 'use_ss', False),
                              esm_cache_dir=esm_cache, verbose=False)
        ds_va = RBP296Dataset(DIPS_ROOT, va_ids, embedder=emb, use_pssm=P.use_pssm, use_dssp=P.use_dssp,
                              use_hmm=getattr(P, 'use_hmm', False), use_ss=getattr(P, 'use_ss', False),
                              esm_cache_dir=esm_cache, verbose=False)
        ds_te = RBP296Dataset(DIPS_ROOT, te_ids, embedder=emb, use_pssm=P.use_pssm, use_dssp=P.use_dssp,
                              use_hmm=getattr(P, 'use_hmm', False), use_ss=getattr(P, 'use_ss', False),
                              esm_cache_dir=esm_cache, verbose=False) if te_ids else None
        ...
        hmm_state = f"{fc['hmm']}/{fc['n']}" if bool(getattr(P, 'use_hmm', False)) else "off"
        ss_state = f"{fc['ss']}/{fc['n']}" if bool(getattr(P, 'use_ss', False)) else "off"
        print(
            f"[features] PSSM={fc['pssm']}/{fc['n']} nonzero={fc.get('pssm_nonzero', 0)}  "
            f"DSSP={fc['dssp']}/{fc['n']} noncoil={fc.get('dssp_noncoil', 0)}  "
            f"HMM={hmm_state}  "
            f"SS-extra={ss_state}  "
...
def _format_binary_metrics(metrics: Dict) -> str:
    return (
        ...
        f"AUROC={_metric_value(metrics, 'auroc'):.4f}  "
...
def _append_metrics_history(path: str, ep: int, loss: float, metrics: Dict):
    if PRIMARY_OBJ == "topk":
        ...
        header = ["run_stamp", "epoch", "split", "loss"] + METRIC_FIELDS + ["thr", "pos_rate", "primary"] + TOPK_FIELDS
    else:
...
def _write_best_metrics_tsv(path: str, epoch: int, metrics: Dict, ckpt_path: str):
    if PRIMARY_OBJ == "topk":
        ...
        header = ["run_stamp", "epoch", "checkpoint"] + METRIC_FIELDS + ["thr", "pos_rate", "primary"] + TOPK_FIELDS
    else:
...
def _make_rbp_eval_loader(ids: List[str], batch_size: int):
    if not ids:
        return None
    emb = None
    seq_mode = str(getattr(P, "sequence_mode", "light")).lower()
    if seq_mode in ("esm", "hybrid"):
        try:
            emb = SiteEmbedder(device=DEVICE, esm_local_dir=ESM_LOCAL_DIR)
        except Exception as e:
            if not getattr(P, "allow_zero_esm_fallback", False):
    ...
    ds = RBP296Dataset(
        ...
        use_pssm=P.use_pssm,
        use_dssp=P.use_dssp,
        use_hmm=getattr(P, 'use_hmm', False),
        use_ss=getattr(P, 'use_ss', False),
        esm_cache_dir=_resolve_esm_cache_dir(for_dips=False),
        verbose=False,
    )
    dl_kw = dict(
    ...
    return DataLoader(ds, shuffle=False, **dl_kw)
...
def _ensure_model_config_compat(cfg, p):
    ...
    defaults = {
        ...
        "l2_focus_topk": int(getattr(p, "l2_focus_topk", getattr(cfg, "eb_pair_topk", 128))),
        "l2_focus_frac": float(getattr(p, "l2_focus_frac", 0.65)),
        "l2_geom_prior_w": float(getattr(p, "l2_geom_prior_w", 0.0)),
        "l2_geom_prior_sigma": float(getattr(p, "l2_geom_prior_sigma", 4.0)),
...
def main():
    ...
    cfg   = _ensure_model_config_compat(build_model_config(P, d_res, d_chain), P)
    ...
    print(
        f"[bridge] GC-EB enable={int(bool(getattr(cfg, 'eb_enable', False)))} "
        f"topk={getattr(cfg, 'eb_topk', 0)}",
        flush=True,
    )
    print(
        f"[pair-head] type={getattr(cfg, 'pair_head_type', 'legacy')} "
    ...
    for ep in range(P.epochs):
        ...
        if PRIMARY_OBJ == "topk":
            vm = eval_topk_residue(model, dl_va)
        elif PRIMARY_OBJ == "binary":
            vm = eval_binary(model, dl_va)
            if bool(getattr(P, "report_topk_metrics", True)):
                tk = eval_topk_residue(model, dl_va)
                for k in TOPK_FIELDS:
                    if k in tk:
                        vm[k] = tk[k]
                vm["topk_primary"] = tk.get("primary", 0.0)
                vm["ager"] = tk.get("ager", False)
        else:
            vm = eval_binary(model, dl_va)
        ema.restore(model)
        ...
        if PRIMARY_OBJ == "binary" and "auprc" in vm:
            tk_msg = ""
            if "topk_score" in vm:
                tk_msg = f"  TopKScore={vm.get('topk_score',0):.4f}  P@K10={vm.get('precision_K10',0):.4f}  R@L/10={vm.get('recall_L10',0):.4f}"
            print(f"[ep {ep_label:02d}] loss={val_loss:.4f}  {_format_binary_metrics(vm)}  thr={vm.get('thr',0.5):.3f}  pos={vm.get('pos_rate',0):.3f}{tk_msg}  time={time.time()-t0:.1f}s  best_ep={best_epoch}", flush=True)
        elif PRIMARY_OBJ == "topk":
        ...
        if ps > best_score:
            ...
            torch.save({"epoch": ep_label, "model_state": model.state_dict(),
                        ...
                        "run_stamp": RUN_STAMP, "metric": BEST_TAG}, RUN_BEST_CKPT)
            ema.restore(model)
    ...
    test_metrics = {}
    if promoted and PRIMARY_OBJ in ("binary", "topk") and dl_te is not None and os.path.exists(GLOBAL_BEST_CKPT):
        try:
            ck = torch.load(GLOBAL_BEST_CKPT, map_location=DEVICE)
            model.load_state_dict(ck["model_state"])
            model.eval()
            if PRIMARY_OBJ == "topk":
                test_metrics = eval_topk_residue(model, dl_te)
            else:
                val_thr = float((ck.get("val_metrics", {}) or {}).get("thr", 0.5))
                test_metrics = eval_binary(model, dl_te, fixed_thr=val_thr)
            _write_best_metrics_tsv(RUN_TEST_METRICS_TSV, best_epoch, test_metrics, GLOBAL_BEST_CKPT)
            if PRIMARY_OBJ == "topk":
                print(f"[test-metrics] Recall@L/5={test_metrics.get('recall_L5',0):.4f}  Recall@L/10={test_metrics.get('recall_L10',0):.4f}  Precision@K10={test_metrics.get('precision_K10',0):.4f}  Hit@K20={test_metrics.get('hit_K20',0):.4f}  Enrich@K10={test_metrics.get('enrichment_K10',0):.2f}  TopKScore={test_metrics.get('topk_score',0):.4f}  metrics_tsv={RUN_TEST_METRICS_TSV}", flush=True)
            else:
                print(
                    f"[test-metrics] {_format_binary_metrics(test_metrics)}  "
                    f"thr={test_metrics.get('thr', 0.5):.3f}  metrics_tsv={RUN_TEST_METRICS_TSV}",
                    flush=True,
                )
        except Exception as e:
